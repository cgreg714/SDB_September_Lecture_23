...
class Store {
    ...
    static createNewStore(name, city, state, inventory, balance, expense, profit, tax_paid) { //create a gap for information by removing salesTax from the parameters
        let stateTax = salesTax.find(salesTax => salesTax.state === state)
        //let stateTax = salesTax.filter((salesTax) => salesTaxstate === this.state)   //method to locate the state appropriate sales tax
